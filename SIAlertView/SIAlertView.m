...
- (void)validateLayout
{
    ...
    if (self.items.count > 0) {
        ...
        if (self.items.count == 2 && self.buttonsListStyle == SIAlertViewButtonsListStyleNormal) {
            ...
            UIButton *button = (self.buttons.count > 0) ? self.buttons[0] : [[UIButton alloc] init];
            button.frame = CGRectMake(CONTENT_PADDING_LEFT, y, width, BUTTON_HEIGHT);
            button = (self.buttons.count > 1) ? self.buttons[1] : button;
...
- (UIButton *)buttonForItemIndex:(NSUInteger)index
{
    ...
    NSBundle *bundle = [NSBundle bundleForClass:SIAlertView.self];
    switch (item.type) {
